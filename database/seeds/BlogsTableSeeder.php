...
class BlogsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for($i = 0; $i < 150; $i++) {
            $blog = [
                'blog_title'    => $titles[rand(0, $titlesEnd)],
                'blog_content'  => $faker->paragraph,
                'blog_img'      => $imgs[rand(0, $imgsEnd)]
            ];
            $blog['blog_views'] = rand(0, 100000);
            $blog['blog_comments'] = rand(0, 100);
            $blog['blog_shares'] = rand(0, 1000);
            $blog['blog_likes'] = rand(0, 10000);
