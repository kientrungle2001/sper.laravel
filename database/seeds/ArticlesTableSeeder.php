...
class ArticlesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for($i = 0; $i < 150; $i++) {
            $article = [
                'article_title'    => $titles[rand(0, $titlesEnd)],
                'article_content'  => $faker->paragraph,
                'article_img'      => $imgs[rand(0, $imgsEnd)]
            ];
            $article['article_views'] = rand(0, 100000);
            $article['article_comments'] = rand(0, 100);
            $article['article_shares'] = rand(0, 1000);
            $article['article_likes'] = rand(0, 10000);
            $article['article_alias'] = str_replace(' ', '_', $article['article_title']);
            $article['article_author'] = 'C5F84AFA-24DD-4B88-BD79-4239746338D4';
            $article['article_category_id'] = '0198FB52-D591-48A5-A968-3A82EFD7E9DA';
